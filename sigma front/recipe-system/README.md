...
`RECIPE-*` packets are linear implementation procedures for a known task shape.
`list-page` remains the pilot implementation recipe for an Angular list page
and its direct list/filter/ListVM contract.

A phase packet establishes or verifies contracts. A task recipe implements an
approved shape. Broad features normally use several phase packets and may also
use one task recipe.
...
```powershell
.\Generate-SigmaRecipes.ps1 -Recipe list-page
```
